...
class EchoEngine():
    ...
    def __init__(self) -> None:  
        ...
        try:
            ...
            self.temperature = float(self.config.get("CALIBRATION_TEMPERATURE", 1.5))
            self.calibration_epsilon = 1e-8
            print(f"Calibration temperature loaded: {self.temperature}", flush=True)
        except:
    ...
    ########################################################################################
    def apply_temperature_scaling(self, predictions):
        """
        Applies temperature scaling to model outputs.

        If predictions already look like probabilities, convert them to pseudo-logits
        using log(p), then apply temperature scaling. This preserves ranking while
        making confidence scores softer or sharper.
        ...
        # If output already looks like probabilities, convert to pseudo-logits
        if tf.reduce_all(predictions >= 0.0) and tf.reduce_all(predictions <= 1.0):
            prediction_sum = tf.reduce_sum(predictions).numpy()
            if abs(prediction_sum - 1.0) < 1e-3:
                predictions = tf.math.log(tf.clip_by_value(predictions, self.calibration_epsilon, 1.0))

        scaled_logits = predictions / self.temperature
        calibrated_probs = tf.nn.softmax(scaled_logits)
    ...
    ########################################################################################
    def predict_class(self, calibrated_probs):
        calibrated_probs = tf.squeeze(calibrated_probs)

        predicted_index = int(tf.argmax(calibrated_probs).numpy())
        predicted_class = self.class_names[predicted_index]
        predicted_probability = 100.0 * calibrated_probs[predicted_index].numpy()
    ...
    def on_message(self, client, userdata, msg):
        ...
        try:   
            ...
            if(audio_event['audioFile'] == "Recording_Mode"): # classic model
                ...
                # Apply calibration
                calibrated_probs = self.apply_temperature_scaling(predictions)

                # Predict class and calibrated probability
                predicted_class, predicted_probability = self.predict_class(calibrated_probs)
    ...
    def echo_api_send_detection_event(self, audio_event, sample_rate, predicted_class, predicted_probability):
        ...
        "calibrated_confidence": predicted_probability,
    ...
    def predict_on_audio(self, binary_audio):
        with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as temp_audio_file:
            ...
            # Apply calibration to prediction output
            calibrated_probs = self.apply_temperature_scaling(predictions[0]).numpy()

            top_two_prob_indices = np.argsort(calibrated_probs)[-2:]
            top_two_prob_values = calibrated_probs[top_two_prob_indices]
