...
class CommsManager():
    ...
    def initialise_communications(self):
        ...
        self.mqtt_client.connect(os.environ['MQTT_CLIENT_URL'], int(
            os.environ['MQTT_CLIENT_PORT']))
        ...
        try:
            ...
            file_path = os.path.join(
                root_dir, 'Data', 'Simulator', 'echo_credentials.json')
        ...
        try:
            ...
            print(
                f"Found echo store database names: {myclient.list_database_names()}", flush=True)
        except:
    ...
    def mqtt_send_random_audio_msg(self, animal, predicted_lla, closest_mic, min_error) -> None:
        ...
        print("DEBUG audio_file extension:",
              audio_file.split(".")[-1], flush=True)
        ...
        (rc, mid) = self.mqtt_client.publish(
            os.environ['MQTT_PUBLISH_URL'], MQTT_MSG)

        logger1.info(
            f'Vocal message sent {animal.getUUID()} time: {timestamp} species: {species_name}')
        print(
            f'Vocal message sent {animal.getUUID()} time: {timestamp} species: {species_name}', flush=True)
    ...
    def mqtt_send_recording_msg(self, msg, mode) -> None:
        ...
        # publish the audio message on the queue
        (rc, mid) = self.mqtt_client.publish(
            os.environ['MQTT_PUBLISH_URL'], msg.payload)
