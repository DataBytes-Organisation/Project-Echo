...
watchDirectory = '/mnt/recordings/pre-processed' 
import_directory = f'../Experiments/0_InitialProcessing/combined'
export_audio = '/mnt/recordings/onset/audio'
export_spectrogram = '/mnt/recordings/onset/spectrograms'

#configure paths
#mic = '1'
#'/mnt/recordings/pre-processed' 
    ...
    if file_name.endswith('.wav'):
        ...
        if init == False:
            ...
            if len(parts) == 2:  # This checks if the split result is exactly two parts (id and number.wav)
                ...
                new_file_path = os.path.join(export_audio, new_file_name)
        ...
        # Save the plot as a JPEG image
            ...
            print(".")
            #output_file_name = f'{export_spectrogram}/{os.path.splitext(new_file_name)[0]}.jpg'

        else:
            print("Couldn't determine the start vocalization time.")
            #output_file_name = f'{export_spectrogram}/{os.path.splitext(new_file_name)[0]}.jpg'
    ...
    def on_any_event(self, event):
        ...
        if event.event_type == 'created':
            ...
            if event.src_path in self.created_files:
                ...
                onsetDetection(event.src_path)
                #Perform the onset detection once in here
            else:
                # This could happen if a file is modified before the creation event is registered
                self.created_files.add(event.src_path)
                print("Watchdog received created event - %s (inferred)." % event.src_path)     
