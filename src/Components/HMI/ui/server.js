...
app.post("/api/create-checkout-session", async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      success_url:
        "http://localhost:9000/donation-success?session_id={CHECKOUT_SESSION_ID}", //`${process.env.CLIENT_URL}`,
...
app.get("/donation-success", async (req, res) => {
  ...
  try {
    ...
    res.send(
      `<script>alert("Donation successful! Thank you."); window.location.href = "/";</script>`,
    );
  } catch (err) {
...
// =======================
const donationClient = new MongoClient(
  ...
  if (validationResult.result) {
    ...
    transporter.sendMail(mailOptions, function (error, info) {
      if (error) {
        ...
        return res.send(
          '<script> alert("user query sent! Please check your mailbox for further communication"); window.location.href = "/"; </script>',
        );
      }
    });
  } else {
    return res
      .status(400)
      .send("<script> alert(`Sender's email is not valid!`)</script>");
  }
});
      ...
      if (response && response.status === "success") {
        ...
        transporter.sendMail(mailOptions, function (error, info) {
          if (error) {
            ...
            console.log("Email sent: " + info.response);
            return res.send(
              '<script> alert("Temporary credential granted! Please check your mailbox."); window.location.href = "/login"; </script>',
            );
          }
        });
      } else {
...
app.get(["/admin/*", "/map", "/requests", "/notifications"], checkUserSession);
...
app.get("/admin-nodes-temp", (req, res) => {
  return res.sendFile(
    path.join(__dirname, "public/admin/admin-nodes-temp.html"),
  );
});
//cloud page
...
app.get("/admin-api-explorer", (req, res) => {
  return res.sendFile(path.join(__dirname, "public/admin/api-explorer.html"));
});
...
app.post("/api/submit", async (req, res) => {
  ...
  try {
    ...
    if (axiosResponse.status === 201) {
      console.log(
        "Status Code: " + axiosResponse.status + " " + axiosResponse.statusText,
      );
      res
        .status(201)
        .send(
          `<script> window.location.href = "/login"; alert("Request Submitted successfully");</script>`,
        );
    } else {
      res
        .status(400)
        .send(
          `<script> window.location.href = "/login"; alert("Ooops! Something went wrong");</script>`,
        );
    }
  } catch (error) {
  ...
  try {
    ...
    if (axiosResponse.status === 200) {
      ...
      res
        .status(200)
        .send(
          `<script> window.location.href = "/login"; alert("Request data updated successfully");</script>`,
        );
    } else {
      res
        .status(400)
        .send(
          `<script> window.location.href = "/login"; alert("Ooops! Something went wrong with updating request table");</script>`,
        );
    }
  } catch (error) {
    console.error(error.data);
...
app.patch("/api/updateConservationStatus/:animal", async (req, res) => {
  ...
  try {
    ...
    const axiosResponse = await axios.patch(
      `${API_BASE_URL}/hmi/api/updateConservationStatus`,
      JSON.stringify(schema),
      {
        headers: {
          Authorization: `Bearer ${token}`,
    ...
    if (axiosResponse.status === 200) {
      ...
      res
        .status(200)
        .send(
          `<script> window.location.href = "/login"; alert("Species Data updated successfully");</script>`,
        );
    } else {
      res
        .status(400)
        .send(
          `<script> window.location.href = "/login"; alert("Ooops! Something went wrong with updating species data");</script>`,
        );
    }
  } catch (error) {
    console.error(error.data);
...
app.get("/api/requests", async (req, res) => {
  try {
    ...
    const axiosResponse = await axios.get(`${API_BASE_URL}/hmi/requests`, {
...
app.get("/welcome", async (req, res) => {
  try {
    ...
    res.send(
      `<script> alert("No user info detected! Please login again"); window.location.href = "/login"; </script>`,
    );
  }
});
...
const suspendOrBlockUser = async (identifier, action) => {
  ...
  try {
    ...
    const status =
      action === "suspend" ? "suspended" : action === "block" ? "banned" : null;
    if (!status) {
    ...
    console.log(
      `User with ${ObjectId.isValid(identifier) ? "ID" : "email"} ${identifier} has been ${status}.`,
    );
