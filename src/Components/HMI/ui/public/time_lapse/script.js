...
// Create vector sources and layers for animal detections (red dots) and survey sites (grey dots)
const animalSource = new ol.source.Vector(); // Red dots (animal detections)
const animalLayer = new ol.layer.Vector({
  source: animalSource,
});

const surveySource = new ol.source.Vector(); // Grey dots (survey sites)
...
// Function to add a marker for animal detection (red dot)
...
// Function to add a marker for survey site (grey dot)
...
// Sample data for animal locations (red dots) and survey sites (grey dots)
...
// Reset map and clear all animal markers
function resetMap() {
  animalSource.clear(); // Clear red dots (animal detections)
}

// Event listener for the "Recentre Map" button
document.getElementById('recentre-map-btn').addEventListener('click', resetMap);
