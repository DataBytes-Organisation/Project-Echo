...
##################################################################################################
# Echo Engine with IoT MQTT Support
#
# This extends echo_engine.py with IoT device integration.
# Once tested, merge the IoT methods into echo_engine.py and update execute().
#
# Changes from echo_engine.py:
#   - Added: on_iot_connect, on_iot_disconnect, on_iot_subscribe, on_iot_message
#   - Added: start_iot_mqtt_listener
#   - Modified: execute() calls start_iot_mqtt_listener() AFTER class_names loaded
#   - Config: reads IOT_MQTT_BROKER, IOT_MQTT_PORT, IOT_MQTT_TOPIC from echo_engine.json
#
...
class EchoEngine():
    ...
    def on_iot_message(self, client, userdata, msg):
        ...
        try:
            ...
            audio_binary = base64.b64decode(payload["audio_file"])
            image, processed_audio, sample_rate = self.combined_pipeline(audio_binary, "Recording_Mode")

            image = tf.expand_dims(image, 0)
            image_list = image.numpy().tolist()

            # Run inference via TF Serving
            data = json.dumps({"signature_name": "serving_default", "inputs": image_list})
            url = self.config['MODEL_SERVER']
            headers = {"content-type": "application/json"}
            json_response = requests.post(url, data=data, headers=headers)
