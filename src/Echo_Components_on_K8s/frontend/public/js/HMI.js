...
export function updateAnimalMovementLayerFromPastData(hmiState, results) {
  ...
  for (const key in updateDict) {
    let event = convertJSONtoAnimalMovementEvent(
      hmiState,
      latestSimAnimals[key]
    );
    hmiState.movementEvents[event.animalId] = event;
  }
...
function addAllTruthFeatures(hmiState) {
  ...
  for (const key in hmiState.movementEvents) {
    ...
    if (entry.animalDiet === "herbavore" || entry.animalDiet === "frugivore") {
      iconPath =
        "./../images/sim/" + getIconName(entry.animalStatus, entry.animalType);
    } else {
      iconPath =
        "./../images/Predator/sim/" +
...
function addmicrophones(hmiState) {
  addMicrophonesByLayer(
    hmiState,
    "mic_layer_9",
    "./../images/Microphone - 3-ai-9.png"
  );
  addMicrophonesByLayer(
    hmiState,
    "mic_layer_8",
    "./../images/Microphone - 3-ai-8.png"
  );
  addMicrophonesByLayer(
    hmiState,
    "mic_layer_7",
    "./../images/Microphone - 3-ai-7.png"
  );
  addMicrophonesByLayer(
    hmiState,
    "mic_layer_6",
    "./../images/Microphone - 3-ai-6.png"
  );
  addMicrophonesByLayer(
    hmiState,
    "mic_layer_5",
    "./../images/Microphone - 3-ai-5.png"
  );
  addMicrophonesByLayer(
    hmiState,
    "mic_layer_4",
    "./../images/Microphone - 3-ai-4.png"
  );
  addMicrophonesByLayer(
    hmiState,
    "mic_layer_3",
    "./../images/Microphone - 3-ai-3.png"
  );
  addMicrophonesByLayer(
    hmiState,
    "mic_layer_2",
    "./../images/Microphone - 3-ai-2.png"
  );
  addMicrophonesByLayer(
    hmiState,
    "mic_layer_1",
    "./../images/Microphone - 3-ai-1.png"
  );
...
function stepMicAnimation(hmiState) {
  ...
  var nextLayer = findMapLayerWithName(
    hmiState,
    "mic_layer_" + micAnimFrameIndex
  );
  nextLayer.setVisible(true);

  var currentLayer = findMapLayerWithName(
    hmiState,
    "mic_layer_" + currentIndex
  );
    ...
    if (feature) {
      ...
      if (values.isMic) {
        ...
        img.onload = function () {
          ...
          document.getElementById("mic_desc_img").src =
            "../../images/bio/mic_bio_" + dice + ".png";
        };
        ...
        document.getElementById("mic_markup_date").innerHTML =
          dateFormat.toUTCString();
        ...
        if (values.animalSpecies) {
          ...
          var result = sample_data.find(
            ({ species }) =>
              species.toLowerCase() === values.animalSpecies.toLowerCase()
          );
          if (result) {
            const img = new Image();
            img.onload = function () {
              ...
              document.getElementById("desc_img").src =
                "../../images/bio/" + result.common.toLowerCase() + "-bio.png";
            };
            img.onerror = function () {
              let dice = Math.floor(Math.random() * 5) + 1;
              document.getElementById("desc_img").src =
                "../../images/bio/not_available_" + dice + "-bio.png";
            };
            img.src =
              "../../images/bio/" + result.common.toLowerCase() + "-bio.png";
            ...
            document.getElementById("desc_confidence").innerText =
              values.animalConfidence + "%";
            ...
            document.getElementById("desc_confidence").innerText =
              values.animalConfidence + "%";
            document.getElementById("desc_species").innerText =
              values.animalSpecies;
            document.getElementById("desc_summary").innerText =
              "Bio data coming soon.";
            let summary = document.getElementById("desc_details");
            summary.innerHTML = "";
          }
          ...
          document.getElementById("markup_details").innerHTML =
            values.animalType +
            " | " +
            values.animalDiet +
            " | " +
            statusPrintLookup[values.animalStatus];
          document.getElementById("markup_loc_lon").innerHTML =
            values.animalLon;
          document.getElementById("markup_loc_lat").innerHTML =
            values.animalLat;
          document.getElementById("markup_confidence").innerHTML =
            values.animalLocConfidence + "%";
          document.getElementById("markup_date").innerHTML =
            dateFormat.toUTCString();
...
function updateTruthEvents(hmiState) {
  retrieveTruthEventsInTimeRange(
    hmiState.currentTime - 5,
    hmiState.currentTime
  ).then((res) => {
...
function purgeTruthEvents(hmiState) {
  ...
  for (const key in hmiState.movementEvents) {
    ...
    if (hmiState.liveEventCutoff > event.timestamp) {
      let layerName = deriveTruthLayerName(
        event.animalStatus,
        event.animalType
      );
      let layer = findMapLayerWithName(hmiState, layerName);
...
var textIndicatorOfAudiPlaying =
  document.getElementsByClassName("playback_indicator")[0];
...
var recordingControls =
  document.getElementsByClassName("recording_controls")[0];
...
function testEncoding(base64String) {
  ...
  const float32Array = new Float32Array(
    binaryData.length / Float32Array.BYTES_PER_ELEMENT
  );
...
export function checkAudioDurationThreshold(duration) {
  ...
  if (timers[2] === MAX_RECORDING_TIME_S) return true;
  else return false;
}
