...
### Spectrogram Augmentation (`augment.py`)

`SpecAugment` (in `augment.py`) masks random frequency/time strips of the
spectrogram, per [SpecAugment](https://arxiv.org/abs/1904.08779). It
supports two ways of sizing each masked strip:

- **Ratio API** (`freq_mask_ratio` / `time_mask_ratio`) — strip width is a
  fraction of the spectrogram's total size on that axis, so mask size
  scales with clip length/resolution instead of being a fixed pixel
  count. Used by the `default`/`light`/`heavy` presets in
  `config/augmentation/`.
- **Legacy pixel API** (`freq_mask_param` / `time_mask_param`) — a fixed
  max width in bins/timeframes, used only by the
  `original_unfixed_reference` preset to reproduce the original,
  uncapped masking behaviour for comparison purposes.

Exactly one of each pair must be supplied per axis (the constructor
raises `ValueError` otherwise). `max_total_time_ratio` caps the
*cumulative* fraction of timeframes masked across all time strips
combined, as a safety net against over-masking short clips; it has no
effect on frequency masking. `mask_value` selects the fill value for
masked regions (currently only `"zero"` is implemented).

See each preset's own comments in `config/augmentation/*.yaml` for the
specific ratios/parameters chosen and why.
