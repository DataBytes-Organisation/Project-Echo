...
const scriptPath = path.join(publicDir, "features", "sensor-health", "sensor-health.js");
const chartsPath = path.join(publicDir, "features", "sensor-health", "charts.js");
const httpClientPath = path.join(publicDir, "shared", "http", "http-client.js");

function loadSensorScriptSource(source) {
  const knownSpecifiers = ["/shared/http/routes.js", "/shared/ui/hmi-utils.js"];
  const stripped = source.replace(
    new RegExp(
      "^\\s*import\\s*\\{[^}]*\\}\\s*from\\s*[\"'](" +
        knownSpecifiers.map((s) => s.replace(/\//g, "\\/")).join("|") +
        ")[\"'];?\\s*$",
      "gm"
    ),
    ""
  );
  const unresolved = stripped.match(/^\s*import[^\n]*$/gm);
  if (unresolved?.length) {
    throw new Error(
      "script.js declares imports the sensor-health test harness does not stub: " +
        unresolved.join(" | ")
    );
  }
  return stripped;
...
function sensorRouteFakes(fetchImpl) {
  ...
  return {
    retrieveSensorUpdates: () => request("/sensors/updates"),
    retrieveSensorAlerts: () => request("/sensors/alerts"),
    retrieveRecentReboots: (limit = 50) => request(`/sensors/reboots/recent?limit=${limit}`),
    retrieveSensorSettings: () => request("/sensors/__default__/settings"),
    updateSensorSettings: (body) => request("/sensors/__default__/settings", {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body),
    }),
    rebootSensor: (sensorId, reason) => request(`/sensors/${encodeURIComponent(sensorId)}/reboot`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ reason }),
    }),
    getApiErrorMessage: (error, fallback = "Request failed.") => error?.message || fallback,
  };
}
...
async function loadDashboardPage({ items, pathname = "/admin/sensor-health.html" }) {
  ...
  const context = {
    ...
    ...sensorRouteFakes(fetchImpl),
  };
  context.window = context;
  context.globalThis = context;

  const httpClient = await readFile(httpClientPath, "utf8");
  const source = loadSensorScriptSource(await readFile(scriptPath, "utf8"));
