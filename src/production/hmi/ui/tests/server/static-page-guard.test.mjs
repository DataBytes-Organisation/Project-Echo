...
test("unauthenticated /pages/map/index.html redirects to /login like /map", async () => {
  const app = createApp({ redisClient: emptyRedis() });
  await withServer(app, async (baseUrl) => {
    for (const route of ["/map", "/index.html", "/pages/map/index.html"]) {
...
test("authenticated /pages/map/index.html still serves the map page", async () => {
  const app = createApp({
    checkUserSession: (req, res, next) => next(),
    redisClient: emptyRedis(),
  });
  await withServer(app, async (baseUrl) => {
    const response = await fetch(`${baseUrl}/pages/map/index.html`, { redirect: "manual" });
