...
try:
    ...
    melspectrogram_to_cam = None
    print(f"helpers.melspectrogram_to_cam import skipped: {e}", flush=True)
...
class EchoEngine():
    ...
    def __init__(self) -> None:  
        ...
        try:
            file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'light_echo_engine.json')
        ...
        try:
            file_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'light_echo_credentials.json')
            with open(file_path, 'r') as f:
        ...
        # Load EfficientNetV2 TFLite model and supporting files
        try:
            self.eff_model_dir = Path(os.path.dirname(os.path.abspath(__file__))) / "Integrate_EfficientNetV2_Engine" / "_trained_models"

            self.eff_tflite_model_path = self.eff_model_dir / "efficientnetv2_project_echo.tflite"
            self.eff_class_mapping_path = self.eff_model_dir / "class_mapping.json"
            self.eff_preprocess_config_path = self.eff_model_dir / "preprocess_config.json"

            self.eff_interpreter = tf.lite.Interpreter(model_path=str(self.eff_tflite_model_path))
            self.eff_interpreter.allocate_tensors()

            self.eff_input_details = self.eff_interpreter.get_input_details()
            self.eff_output_details = self.eff_interpreter.get_output_details()

            with open(self.eff_class_mapping_path, "r", encoding="utf-8") as f:
                self.eff_class_mapping = json.load(f)

            with open(self.eff_preprocess_config_path, "r", encoding="utf-8") as f:
                self.eff_preprocess_config = json.load(f)

            print("EfficientNetV2 TFLite model loaded successfully.", flush=True)
            print(f"EfficientNetV2 input shape: {self.eff_input_details[0]['shape']}", flush=True)
            print(f"EfficientNetV2 output shape: {self.eff_output_details[0]['shape']}", flush=True)

        except Exception as e:
            print(f"Failed to load EfficientNetV2 TFLite model: {e}", flush=True)
    ...
    def efficientnetv2_preprocess_audio_bytes(self, audio_clip):
        """
        Preprocess raw audio bytes for the EfficientNetV2 TFLite model.
        ...
        target_sr = self.eff_preprocess_config["target_sr"]
        duration_s = self.eff_preprocess_config["duration_s"]
        n_mels = self.eff_preprocess_config["n_mels"]
        hop_length = self.eff_preprocess_config["hop_length"]
        fmin = self.eff_preprocess_config["fmin"]
        fmax = self.eff_preprocess_config["fmax"]

        audio, sample_rate = librosa.load(file, sr=target_sr, mono=True)
        ...
        # Same normalisation used in the validated predictor
        ...
        input_shape = tuple(self.eff_input_details[0]["shape"])

        if tuple(x.shape) == input_shape:
            return x.astype(np.float32), audio, target_sr

        # In case future TFLite export expects NHWC
        x_nhwc = np.transpose(x, (0, 2, 3, 1))

        if tuple(x_nhwc.shape) == input_shape:
            return x_nhwc.astype(np.float32), audio, target_sr

        raise ValueError(
            f"EfficientNetV2 input shape mismatch. "
            f"Expected {input_shape}, got NCHW {x.shape} or NHWC {x_nhwc.shape}"
        )


    def efficientnetv2_tflite_predict_from_audio_bytes(self, audio_clip):
        """
        Run EfficientNetV2 TFLite inference from raw audio bytes.
        """

        input_index = self.eff_input_details[0]["index"]
        output_index = self.eff_output_details[0]["index"]

        x, processed_audio, sample_rate = self.efficientnetv2_preprocess_audio_bytes(audio_clip)

        self.eff_interpreter.set_tensor(input_index, x)
        self.eff_interpreter.invoke()

        output = self.eff_interpreter.get_tensor(output_index)
        ...
        index_to_label = self.eff_class_mapping["index_to_label"]
        ...
        return predicted_class, predicted_probability, processed_audio, sample_rate, top_predictions
    ...
    def on_message(self, client, userdata, msg):
        ...
        try:   
            ...
            if audio_event["audioFile"] == "EfficientNetV2_TFLite_Mode":
                ...
                if melspectrogram_to_cam is not None:
