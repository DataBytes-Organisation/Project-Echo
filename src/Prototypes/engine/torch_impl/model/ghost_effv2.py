...
class ConvBnAct(nn.Module):
	def __init__(self, in_channels, out_channels, kernel_size=3, stride=1, padding=0, groups=1, relu=True):
		...
		self.conv = nn.Conv2d(
			in_channels, out_channels, kernel_size, stride, padding=padding, groups=groups, bias=False
		)
...
class FusedMBConv(nn.Module):
	def __init__(self, in_channels, out_channels, stride, expand_ratio, drop_rate):
		...
		if expand_ratio != 1:
			...
			layers.append(
				ConvBnAct(in_channels, hidden_dim, kernel_size=3, stride=stride, padding=1, groups=in_channels)
			)
