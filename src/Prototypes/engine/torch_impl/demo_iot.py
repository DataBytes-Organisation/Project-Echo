...
def preprocess_with_librosa(waveform: np.ndarray, pp_data: dict) -> np.ndarray:
	...
	Returns a 2D spectrogram (Freq, Time).
	Note: The CLI script returns 4D, but we handle dimensions in process_single_chunk for flexibility.
	"""
...
def handle_audio_upload(filepath):
	...
	# --- CRITICAL CHANGE: Use librosa.load ---
	# This ensures normalization, stereo-to-mono, and resampling match the CLI script 100%
	try:
	...
	if ASSETS["error"]:
		...
		with gr.Row():
			with gr.Column(scale=1):
				...
				# Changed type="numpy" to type="filepath" for 1:1 parity with CLI
