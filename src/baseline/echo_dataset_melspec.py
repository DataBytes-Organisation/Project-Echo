...
class EchoDatasetMelspec(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if duration > htsat_config.CLIP_LENGTH:
            offset = random.randint(0, int(math.floor(duration-htsat_config.CLIP_LENGTH))) // step
            offset = offset * step
        else:
            offset = 0
        ...
        if not os.path.exists(melspec_file):
            ...
            melspec = librosa.feature.melspectrogram(waveform, 
                                                sr=htsat_config.sample_rate, 
                                                S=None, 
                                                n_fft=htsat_config.window_size, 
                                                hop_length=htsat_config.hop_size, 
                                                win_length=None, 
                                                window='hann', 
                                                center=True, 
                                                n_mels=htsat_config.mel_bins,
                                                pad_mode='constant',
                                                fmin=htsat_config.fmin,
                                                fmax=htsat_config.fmax,
                                                power=2.0)
